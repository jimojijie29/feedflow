...
/** Shows a banner when source refresh fails, with cookie-sync guidance for authentication errors.
...
export function RefreshErrorBanner(): JSX.Element | null {
  ...
  const cookieFailure = /Cookie\s*(?:已过期|无效|失效)|未登录|登录(?:已)?(?:过期|失效)|需要重新登录/i.test(error.error || '')
